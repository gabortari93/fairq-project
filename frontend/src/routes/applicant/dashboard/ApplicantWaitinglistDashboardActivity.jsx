...
export default function ApplicantWaitinglistDashboardActivity() {
    ...
    const fetchActivities = async (e) => {
        try {
            const res = await api.get(`/list/${waitingList.id}/application/${application.id}/activities`, {
    ...
    if (activities.length === 0) return <Loading/>
        ...
        <TitleWithDescription title={"Latest activity"}
                              description={"Keep track of all changes on your application."}/>
